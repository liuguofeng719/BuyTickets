...
public class TicketActivity extends BaseActivity {
    ...
    private void setLocation() {
        if (!TextUtils.isEmpty(provinceName) && !TextUtils.isEmpty(cityName)) {
            Call<CityInfoResp<CityVo>> callGPSCity = getApis().getGPSCity(provinceName, cityName).clone();
            callGPSCity.enqueue(new Callback<CityInfoResp<CityVo>>() {
                @Override
                public void onResponse(Response<CityInfoResp<CityVo>> response, Retrofit retrofit) {
                    if (response.isSuccess() && response.body() != null && response.body().isSuccessfully()) {
                        CityVo cityVo = response.body().getCityInfo();
                        if (cityVo != null) {
                            start_city.setText(cityVo.getCityName());
                            startCityName = cityVo.getCityName();
                            startCityId = cityVo.getCityId();
                        }
                    }
                }
        ...
        this.ly_end_city.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (!TextUtils.isEmpty(start_city.getText())) {
                    ...
                    CommonUtils.make(TicketActivity.this, getString(R.string.start_tip_city));
                }
            }
        });
